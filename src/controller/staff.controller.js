...
const {randomStaffId} = require('../helper/ramdon')
  ...
  try {
    ...
    await Staffs.findOne({ where: { tel: tel } }).then(async (data) => {
      ...
      if (validatePassword) {
        ...
        const token = await jwt.sign(statffData, "CLINIC", {
          expiresIn: "120d",
        });
...
exports.getAll = async (req, res) => {
  try {
    await Staffs.findAndCountAll({ order: [["createdAt", "DESC"]] }).then(
      (data) => {
        if (!data) {
...
exports.updateData = async (req, res) => {
  try {
    const { id } = req.params;
    const updateData = await Staffs.update(
      { ...req.body },
      { where: { id: id } }
    );
    if (updateData) {
